...
/**
 * Global replace regexp `reg` with `repl` in `str`
 *
 * @param {String} str
 * @param {String} reg
 * @param {String} repl
...
(exports['reg-replace'] = function jsReplace(str, reg, repl){
  reg = utils.unwrap(reg).first.val;
  reg = new RegExp(reg, 'g');
  str = utils.unwrap(str).first.val;
  repl = utils.unwrap(repl).first.val;
  return new nodes.Literal(str.replace(reg, repl));
}).raw = true;
